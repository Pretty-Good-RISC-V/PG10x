...
class pg10x(pluginTemplate):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.dut_exe = os.path.join(config['PATH'] if 'PATH' in config else "","pg10xsim")
    ...
    def initialise(self, suite, work_dir, archtest_env):
       ...
       self.dump_cmd = 'riscv{0}-unknown-elf-objdump -d {1} > {2}.dump.txt'
    ...
    def build(self, isa_yaml, platform_yaml):
      ...
      # for pg10x start building the '--isa' argument. the self.isa is dutnmae specific and may not be
      # useful for all DUTs
    ...
    def runTests(self, testList):
      ...
      for testname in testList:
          ...
          dump_cmd = self.dump_cmd.format(self.xlen, elf, elf)
	  ...
	  # echo statement.
          if self.target_run:
            ...
            simcmd = self.dut_exe + ' --isa {0} --signature-file {1} {2}'.format(self.isa, sig_file, elf)
          else:
            simcmd = 'echo "NO RUN"'

          # concatenate all commands that need to be executed within a make-target.
          execute = '@cd {0}; {1}; {2}; {3};'.format(testentry['work_dir'], cmd, dump_cmd, simcmd)
